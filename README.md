...
|------|------|-----------|-----------|------------------|--------|-------------|----------------|----------|
| 🥇 **1** | **GPT-4.1-mini** | - | **⚡ 1초** | **⭐⭐⭐ (3개)** | ✅ **안정** | **$160** | **$16K** | $0.4 • $1.6 |
...
**선택 이유:**
- ✅ **최고 속도**(1초): 가드레일 오버헤드 최소화 → 사용자 대기 시간 2초 이내
...
**절대 피해야 할 안티패턴:**
- ❌ 과도한 엔지니어링: Full 모델로 단순 분류 수행
- ❌ 위험한 최적화: minimal 모드로 속도 추구 → 오탐 발생
- ❌ 무계획 전환: 트래픽/비용 분석 없이 모델 변경

**성공 지표:**
- ✅ 가드레일 응답 시간 < 2초 유지
- ✅ 오탐률(false positive) < 1%
- ✅ 미탐률(false negative) = 0% (금융 필수)
- ✅ 월 운영 비용 < 매출의 5%
