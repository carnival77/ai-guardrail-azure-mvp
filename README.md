...
**비즈니스 과제:** 금융 서비스의 AI 가드레일은 **실시간성**(사용자 경험), **정확도**(규제 준수), **비용 효율성**(운영 지속성)의 3가지를 모두 충족해야 합니다.
...
- ✅ **제로 오탐**: 테스트 케이스 100% 정확 판정 → 금융 서비스 신뢰성 확보
...
**⚠️ 중요:** GPT-5-mini의 `minimal` 모드는 political 점수 0.78로 오탐 발생 → **금융 가드레일 사용 금지**

---

#### 💡 현재 배포 모델 (GPT-4.1 Full) 재검토

**현황:**
- 현재 프로젝트: `GPT-4.1` (Full 모델) 사용 중
- 이전 벤치마크: `GPT-4.1-mini` 선정

**문제점:**

| 비교 | GPT-4.1 Full | GPT-4.1-mini | 개선 효과 |
|------|--------------|--------------|-----------|
| **응답 속도** | 2.5초 | 1초 | **⚡ 2.5배 개선** |
| **월 비용 (MVP)** | $1,000 | $160 | **💰 84% 절감** |
| **월 비용 (대규모)** | $100,000 | $16,000 | **💰 연 $1M 절감** |
| **정확도** | 4-5개 (과잉) | 3개 (충분) | ⚖️ 차이 없음 (단순 분류 태스크) |

**권장 조치:**
```yaml
# config/config.yaml
azure_openai_chat_deployment_name: "GPT-4.1-mini"  # GPT-4.1 → mini로 변경
```

**비즈니스 임팩트:**
- 🚀 **사용자 경험 개선**: 가드레일 대기 시간 5초 → 2초 (60% 감소)
- 💰 **운영 비용 최적화**: 연간 $10K (MVP) ~ $1M (대규모) 절감
- ⚡ **시장 경쟁력 확보**: 타사 대비 2배 빠른 응답 속도
